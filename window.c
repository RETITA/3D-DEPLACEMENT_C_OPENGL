...
/*!\brief ball1 te0ture Id */
static GLuint _ballTexId = 0;
/*!\brief ball1 texture Id */
static GLuint _ballTexId1 = 0;
/*!\brief ball2 texture Id */
static GLuint _ballTexId2 = 0;
/*!\brief vie texture Id */
static GLuint _vieTexId = 0;
...
GLfloat vie = 0.01,a = 0.0;
Coord tableau_coord[50];
static Coord player = {0 ,0 ,0 ,0};
...
/*init tableau de coord and type of objet*/
void init_tab(){
  srand(time(NULL));
  int i;
  for(i=0;i < 50;i++){
    
    tableau_coord[i].x = (rand()%(99)+1)-50;
    tableau_coord[i].z = (rand()%(99)+1)-50;
    tableau_coord[i].type = (rand()%3)+1;
...
static void initData(void) {
  ...
  /*initialise le tableau de coord*/
   init_tab();
  ...
  if(s) {
#ifdef __APPLE__
    int mode = s->format->BytesPerPixel == 4 ? GL_BGRA : GL_BGR;
#else
  ...
  /*  creation and parametrization of the ball 0 texture */
  s = IMG_Load("ball3.png");
  ...
  glBindTexture(GL_TEXTURE_2D, 0);
    ...
    /* creation and parametrization of the ball 1 texture */
   s = IMG_Load("ball.png");
  glGenTextures(1, &_ballTexId1);
  glBindTexture(GL_TEXTURE_2D, _ballTexId1);
  glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
  glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
  ...
  glBindTexture(GL_TEXTURE_2D, 0);
    ...
    /* creation and parametrization of the ball 2 texture */
   s = IMG_Load("ball2.png");
  glGenTextures(1, &_ballTexId2);
  glBindTexture(GL_TEXTURE_2D, _ballTexId2);
  glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
  glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
  ...
  glBindTexture(GL_TEXTURE_2D, 0);
    ...
    /* creation and parametrization of the vie texture */
  glGenTextures(1, &_vieTexId);
  glBindTexture(GL_TEXTURE_2D, _vieTexId);
  glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
  glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 1, 2, 0, GL_RGBA, GL_UNSIGNED_BYTE, life);
...
static void idle(void) {
  ...
  if(_keys[KUP]) {
    ...
    player.z += -dt * step * cos(_cam.theta);
    a+= dt * step * cos(_cam.theta);
  }
  if(_keys[KDOWN]) {
    _cam.x += dt * step * sin(_cam.theta);
...
/*
  ...
  coordonées (x, 1, z)
*/
void unObjet(GLfloat x, GLfloat z,GLint type) {
  ...
  gl4duPushMatrix(); {
    ...
    if(_keys[KLEFT])
      gl4duRotatef( a * 180.0f / M_PI, 0, 0, 1);
    if(_keys[KRIGHT])
      gl4duRotatef(- a* 180.0f / M_PI, 0, 0, 1);
    if(_keys[KUP])
      gl4duRotatef(-a * 180.0f / M_PI,1, 0, 0);
    if(_keys[KDOWN])
      gl4duRotatef(a * 180.0f / M_PI,1, 0, 0);
    gl4duSendMatrices();
  } gl4duPopMatrix();
...
/*draw n objets using unObjet*/
void draw_objets(int n){
  int i;
  for(i = 0; i < n; i++){
    if( _cam.x + 1 >= tableau_coord[i].x && _cam.x -1 <= tableau_coord[i].x &&
        _cam.z + 1 >= tableau_coord[i].z && _cam.z -1 <= tableau_coord[i].z)
      {
	if( tableau_coord[i].type == 2.0 ||  tableau_coord[i].type == 3.0)
	  vie += 0.03;
	else if(tableau_coord[i].type == 1.0)
	  vie -= 0.03;
	tableau_coord[i].x = rand()%(99)+1;
	tableau_coord[i].z = rand()%(99)+1;
      }
    
    unObjet(tableau_coord[i].x,tableau_coord[i].z,tableau_coord[i].type);
  } 
}
...
static void draw(void) {
  ...
  gl4duPushMatrix(); {
    ...
    gl4duPushMatrix(); {
      gl4duLoadIdentityf();
      gl4duTranslatef(0.75, 0.7, 0.0);
      gl4duScalef(vie / 5.0, 0.07 / 5.0, 0.07 / 5.0);
      gl4duSendMatrices();
    } gl4duPopMatrix();
    gl4duBindMatrix("projectionMatrix");
  } gl4duPopMatrix();
  ...
  unObjet(player.x ,player.z ,tableau_coord[0].type);
...
static void quit(void) {
  ...
  if(_ballTexId)
   ...
   if(_ballTexId1)
    glDeleteTextures(1, &_ballTexId1);
    if(_ballTexId2)
    glDeleteTextures(1, &_ballTexId2);
     if(_vieTexId)
    glDeleteTextures(1, &_vieTexId);
